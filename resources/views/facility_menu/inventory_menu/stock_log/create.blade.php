...
@section('title', 'Add Stock Log')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h3 class="mb-0">Add New Stock Log</h3>
                    ...
                    <div class="row">
                            ...
                            <select name="supply_list_id" id="supply_list_id"
                                ...
                                @foreach ($supplies as $supply)
                                    <option value="{{ $supply->id }}"
                                        {{ old('supply_list_id') == $supply->id ? 'selected' : '' }}>
                                        {{ $supply->item_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select name="log_type" id="log_type"
                                ...
                                <option value="stock_in" {{ old('log_type') == 'stock_in' ? 'selected' : '' }}>Stock In
                                </option>
                                <option value="stock_out" {{ old('log_type') == 'stock_out' ? 'selected' : '' }}>Stock Out
                                </option>
                            </select>
                            @error('log_type')
                                <small class="text-danger">{{ $message }}</small>
                        ...
                        <div class="col-md-6 form-group mt-3">
                            ...
                            <input type="date" name="log_date" id="log_date"
                                class="form-control @error('log_date') is-invalid @enderror" value="{{ old('log_date') }}">
                            @error('log_date')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
