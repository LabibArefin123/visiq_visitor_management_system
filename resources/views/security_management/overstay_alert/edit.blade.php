...
@section('title', 'Edit Overstay Alert')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h3 class="mb-0">Edit Overstay Alert</h3>
    ...
    <div class="container">
        <div class="card shadow-lg">
            <div class="card-body">
                ...
                @endif
                <form action="{{ route('overstay_alerts.update', $overstayAlert->id) }}" method="POST" data-confirm="edit">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        {{-- Visitor --}}
                            ...
                            <select name="visitor_id" id="visitor_id"
                                ...
                                @foreach ($visitors as $visitor)
                                    <option value="{{ $visitor->id }}"
                                        {{ $overstayAlert->visitor_id == $visitor->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="date" name="visit_date" id="visit_date"
                                ...
                                value="{{ old('visit_date', $overstayAlert->visit_date ? $overstayAlert->visit_date->format('Y-m-d') : '') }}">
                            @error('visit_date')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="date" name="expected_checkout_date" id="expected_checkout_date"
                                ...
                                value="{{ old('expected_checkout_date', $overstayAlert->expected_checkout_date ? $overstayAlert->expected_checkout_date->format('Y-m-d') : '') }}">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="date" name="actual_checkout_date" id="actual_checkout_date"
                                ...
                                value="{{ old('actual_checkout_date', $overstayAlert->actual_checkout_date ? $overstayAlert->actual_checkout_date->format('Y-m-d') : '') }}">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="Pending" {{ $overstayAlert->status == 'Pending' ? 'selected' : '' }}>Pending
                                </option>
                                <option value="Resolved" {{ $overstayAlert->status == 'Resolved' ? 'selected' : '' }}>
                                    Resolved
                                </option>
                            </select>
                            @error('status')
                        ...
                        <div class="col-md-12 form-group">
                            ...
                            <textarea name="remarks" id="remarks" class="form-control @error('remarks') is-invalid @enderror" rows="3">{{ old('remarks', $overstayAlert->remarks) }}</textarea>
                            @error('remarks')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
