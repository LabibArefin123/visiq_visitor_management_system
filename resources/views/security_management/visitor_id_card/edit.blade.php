...
@section('title', 'Edit Visitor ID Card')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h3 class="mb-0">Edit Visitor ID Card</h3>
        <a href="{{ route('visitor_id_cards.index') }}"
...
@section('content')
    <div class="card shadow-lg">
        <div class="card-body">
            ...
            @endif
            <form action="{{ route('visitor_id_cards.update', $visitorIdCard->id) }}" method="POST" data-confirm="edit">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6 form-group">
                        ...
                        <input type="text" name="card_number" id="card_number"
                            ...
                            value="{{ old('card_number', $visitorIdCard->card_number) }}" placeholder="Enter card number"
                            readonly>
                        @error('card_number')
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <select name="holder_type" id="holder_type"
                            ...
                            <option value="visitor"
                                {{ old('holder_type', $visitorIdCard->holder_type) == 'visitor' ? 'selected' : '' }}>
                                Visitor
                            </option>
                        </select>
                        @error('holder_type')
                    ...
                    <div class="col-md-6 form-group mt-3">
                        ...
                        <input type="date" name="issue_date" id="issue_date"
                            ...
                            value="{{ old('issue_date', optional($visitorIdCard->issue_date)->format('Y-m-d')) }}">
                        @error('issue_date')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 form-group mt-3">
                        ...
                        <input type="date" name="expiry_date" id="expiry_date"
                            ...
                            value="{{ old('expiry_date', optional($visitorIdCard->expiry_date)->format('Y-m-d')) }}">
                        @error('expiry_date')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 form-group mt-3">
                        ...
                        <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                            ...
                            <option value="active"
                                {{ old('status', $visitorIdCard->status) == 'active' ? 'selected' : '' }}>
                                Active</option>
                            <option value="inactive"
                                {{ old('status', $visitorIdCard->status) == 'inactive' ? 'selected' : '' }}>
                                Inactive</option>
                            <option value="expired"
                                {{ old('status', $visitorIdCard->status) == 'expired' ? 'selected' : '' }}>
                                Expired</option>
                            <option value="pending"
                                {{ old('status', $visitorIdCard->status) == 'pending' ? 'selected' : '' }}>
                                Pending</option>
                        </select>
                        @error('status')
                            <small class="text-danger">{{ $message }}</small>
                    ...
                    <div class="col-md-12 form-group mt-3">
                        ...
                        <textarea name="remarks" id="remarks" rows="3" class="form-control" placeholder="Enter remarks">{{ old('remarks', $visitorIdCard->remarks) }}</textarea>
                    </div>
                </div>
...
@section('js')
    <script>
        ...
        const selectedHolderType = "{{ $visitorIdCard->holder_type }}";
        const selectedHolderId = "{{ $visitorIdCard->holder_id }}";

        // Fetch holders based on existing type (for editing)
        function loadHolders(type, selectedId = null) {
            if (!type) return;
            fetch(`/get-holders-by-visitor/${type}`)
