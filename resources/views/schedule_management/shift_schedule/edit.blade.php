...
@section('content')
    <div class="container">
        <div class="card shadow-lg mt-3">
            <div class="card-body">
                <form action="{{ route('shift_schedules.update', $shiftSchedule->id) }}" method="POST" data-confirm="edit">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="time" name="start_time" id="start_time"
                                ...
                                value="{{ old('start_time', $shiftSchedule->start_time) }}">
                            @error('start_time')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="time" name="end_time" id="end_time"
                                ...
                                value="{{ old('end_time', $shiftSchedule->end_time) }}">
                            @error('end_time')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="Active" {{ $shiftSchedule->status == 'Active' ? 'selected' : '' }}>Active
                                </option>
                                <option value="Inactive" {{ $shiftSchedule->status == 'Inactive' ? 'selected' : '' }}>
                                    Inactive</option>
                            </select>
                            @error('status')
                                <small class="text-danger">{{ $message }}</small>
