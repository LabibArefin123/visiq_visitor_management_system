...
class VisitorCompanyController extends Controller
{
    ...
    public function indexGroupMembers($visitorId)
    {
        $visitor = Visitor::findOrFail($visitorId);
        $members = VisitorGroupMember::where('visitor_id', $visitorId)->get();
        $totalgroupvisitors = $members->count(); // Count total group members

        return view('visitor_management.visitor_group_member_index', compact('visitor', 'members', 'totalgroupvisitors'));
    }


    public function addGroupMember($visitorId)
    {
        $visitor = Visitor::findOrFail($visitorId);
        return view('visitor_management.visitor_group_member_add', compact('visitor'));
    }

    public function storeGroupMember(Request $request, $visitorId)
    {
        $request->validate([
            'gid' => 'required|string|max:50',
            'name' => 'required|string|max:255',
            'email' => 'required|email|unique:visitor_group_members,email',
            'phone' => 'required|string|max:20',
            'purpose' => 'required|string|max:255',
        ]);
        ...
        return redirect()->route('visitor_group_member.index', $visitorId)
            ->with('success', 'Member added successfully.');
    }

    public function deleteGroupMember($memberId)
    {
        $member = VisitorGroupMember::findOrFail($memberId);
        $member->delete();

        return back()->with('success', 'Member deleted successfully.');
    }

    public function viewGroupMember($memberId)
    {
        $member = VisitorGroupMember::findOrFail($memberId);
        return view('visitor_management.visitor_group_member_view', compact('member'));
    }

    public function editGroupMember($memberId)
    {
        $member = VisitorGroupMember::findOrFail($memberId);
        return view('visitor_management.visitor_group_member_edit', compact('member'));
    }

    public function updateGroupMember(Request $request, $memberId)
    {
        $request->validate([
            'gid' => 'required|string|max:50',
            'name' => 'required|string|max:255',
            'email' => 'required|email|unique:visitor_group_members,email,' . $memberId,
            'phone' => 'required|string|max:20',
            'purpose' => 'required|string|max:255',
        ]);
        ...
        return redirect()->route('visitor_group_member.index', $member->visitor_id)
